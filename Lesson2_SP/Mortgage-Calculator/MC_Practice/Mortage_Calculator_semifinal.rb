...
loop do # main outer loop
  ...
  loop do
    ...
    loop do
      ...
      system 'clear'
      prompt(messages('welcome_2', LANG))
      prompt("#{messages('current user', LANG)} #{name} || #{messages('loan_display', LANG)} #{loan_amount}") # *
      ...
      system 'clear'
      prompt(messages('welcome_2', LANG))
      prompt("#{messages('current user', LANG)} #{name} || #{messages('loan_display', LANG)} #{loan_amount} || APR: #{apr}")
