...
def player_places_piece!(board)
  square = ''
  loop do
    prompt "Please pick an available square:  #{joinor(select_squares(board, EMPTY_MARKER))}: "
    square = gets.to_i
...
def find_at_risk_square(board)
  squares_at_risk = select_squares(board, EMPTY_MARKER).select do |square|
    possible_wins(board, PLAYER_MARKER).any? do |winning_set|
      winning_set.include?(square)
    end
  end

  squares_to_win = select_squares(board, EMPTY_MARKER).select do |square|
    possible_wins(board, COMPUTER_MARKER).any? do |winning_set|
      winning_set.include?(square)
    end
  end
