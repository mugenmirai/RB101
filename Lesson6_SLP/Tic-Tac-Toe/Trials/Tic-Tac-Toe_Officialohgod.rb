...
def tie?(result)
  (result == 'player') || (result == 'computer') ? "=> #{result} is the champion!" : 'tie'
end
...
#        current_round += 1
    ...
    current_player = alternate_player(current_player) if tie?(game_result) == 'tie'
    #current_player = 'player' if tie?(game_result) == 'computer'
    sleep 2
